...
export const load: PageServerLoad = async ({ params }) => {
	...
	if (thingDB.name.endsWith('.txt')) {
		...
		thingDB.name.endsWith('.png') ||
		thingDB.name.endsWith('.jpg') ||
		thingDB.name.endsWith('.jpeg') ||
		thingDB.name.endsWith('.webp') ||
		thingDB.name.endsWith('.gif')
	) {
		return { id: thingID, type: 'image', fileName: thingDB.name };
	} else if (
		thingDB.name.endsWith('.mp4') ||
		thingDB.name.endsWith('.mov') ||
		thingDB.name.endsWith('.mkv')
	) {
